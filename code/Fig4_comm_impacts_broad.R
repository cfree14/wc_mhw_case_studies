...
# Read data
data_orig <- readRDS(data, file=file.path(datadir, "NOAA_PACFIN_1980_2021_commercial_landings.Rds"))
...
data_tot <- data_orig %>%
  ...
  mutate(state=recode(state, "Alaska"="Alaska (below)"),
         state=factor(state, levels=c("California", "Oregon", "Washington", "Alaska (below)")))
...
data_group <- data_orig %>%
  ...
  mutate(period=cut(year, breaks=c(2010, 2013, 2016, 2020), labels=c("Before", "During", "After"), right=T)) %>%
  # Summarize
  group_by(state, mgmt_group_use, period) %>%
  summarise(value_usd=sum(value_usd, na.rm=T)) %>%
  ungroup() %>%
  # Complete
  complete(state, mgmt_group_use, period, fill=list(value_usd=NA)) %>%
  # Calculate percent difference
  group_by(state, mgmt_group_use) %>%
  ...
  mutate(mgmt_group_use=recode(mgmt_group_use,
...
data_group_ordered <- data_group %>%
  mutate(mgmt_group_use=factor(mgmt_group_use, levels=group_order$mgmt_group_use))
...
g3 <- ggplot(data_group_ordered, aes(x=period, y=mgmt_group_use, fill=value_usd_pdiff)) +
  facet_wrap(~state_abbrev, nrow=1) +
  # Plot ratser
  geom_raster() +
  # Plot points
  geom_point(data=data_group %>% filter(period=="Before"),
             mapping=aes(x=period, y=mgmt_group_use, size=value_usd/1e6), inherit.aes = F ) +
  # Labels
...
# Export
ggsave(g, filename=file.path(plotdir, "Fig4_comm_impacts_broad.png"),
