...
data <- data_orig %>%
  ...
  mutate(state=recode(state, "California"="California (below)"),
         state=factor(state, levels=c("California (below)", "Oregon", "Washington", "Alaska")))
...
# Plot data
g2 <- ggplot(data_group, aes(x=period, y=taxa_catg, size=retained_n/1e6, fill=retained_n_pdiff_cap)) +
...
# Export
ggsave(g, filename=file.path(plotdir, "Fig6_rec_impacts_broad.png"),
