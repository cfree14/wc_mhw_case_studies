...
# Species key
spp_key <- data_orig %>%
  select(mgmt_group_use, comm_name, level) %>%
  unique()
...
data <- data_orig %>%
  ...
  mutate(state_abbrev=recode(state,
                             ...
                             "Washington"="WA",
                             "Alaska"="AK"),
         state_abbrev=factor(state_abbrev, levels=c("CA", "OR", "WA", "AK"))) %>%
...
spp_order_key <- data %>%
  ...
  # Summarize
  group_by(mgmt_group_use, comm_name) %>%
  summarize(pdiff_avg=mean(value_usd_pdiff, na.rm=T)) %>%
  ungroup() %>%
  # Order
  arrange(mgmt_group_use, pdiff_avg)
...
g <- ggplot(data, aes(x=period, y=comm_name, fill=value_usd_pdiff_cap)) +
  facet_grid(mgmt_group_use~state_abbrev, space="free_y", scale="free_y") +
  geom_raster() +
  # Labels
...
# Export
ggsave(g, filename=file.path(plotdir, "Fig5_comm_impacts_specific.png"),
